...
const App: React.FunctionComponent<TaskListProps> = ({ taskMap }) => {
  ...
  const numCards = categories.map(category => taskMap.get(category) as TaskCard[]).map(card => card.length).reduce((prev, n) => prev + n);
  return (
    <div>
      <div style={{ display: "flex", flexDirection: "row", marginTop: "1rem" }}>
        <h1>RECOMMENDED SCENARIOS TO TEST</h1>
        <div style={{ marginLeft: "1rem", backgroundColor: "blue", borderRadius: "50%", width: "3.3rem", textAlign: "center", color: "white" }}>
          <h1>{numCards}</h1>
        </div>
      </div>
      <div>
        ...
        {categories.map(category => (
          <React.Fragment>
            <div style={{ display: "flex", flexDirection: "row", marginTop: "2.5rem" }}>
              <h1>{category}</h1>
              <div style={{ marginLeft: "1rem", backgroundColor: "#73c7ff", borderRadius: "50%", width: "3.3rem", textAlign: "center" }}><h1>{taskMap.get(category)?.map(task => task.tasks.length).reduce((prev, n) => prev + n)}</h1></div>
            </div>
